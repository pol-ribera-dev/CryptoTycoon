...
contract myNFT is ERC721Enumerable {
    ...
    function mintAll(address _user) external onlyMaster {
        for (uint256 i = 0; i < (amountBase + amountMulti); i++) {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory json_ = string(
            abi.encodePacked(
                ...
                "},",
                '{"trait_type": "Upgrade_price", "value": ',
                price_.toString(),
                "}]}"
            )
        );
        ...
        return string(abi.encodePacked("data:application/json;base64,", encoded_));
    }

    /// @notice Returns the token cost required to upgrade an NFT.
    /// @param _tokenId NFT identifier.
    /// @return price_ Upgrade cost.
    function getPrice(uint256 _tokenId) external view returns (uint256 price_) {
        _requireOwned(_tokenId);
        TokenData memory data_ = IdToData[_tokenId];
        bool type_ = data_.multi;
        uint8 lvl_ = data_.lvl;
